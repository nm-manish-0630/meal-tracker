...
- Photo *reassignment* to a different meal is not a feature the app has — there's no edit flow that moves a photo between meals, so this isn't an open question or a gap in scope, just a case that doesn't arise. Only newly-confirmed photos ever enqueue
